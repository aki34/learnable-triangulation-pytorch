...
retval = {
    # 'subject_names': [f'S{i}' for i in range(1, 9)],
    'subject_names': ['S1', 'S2'],
    # 'camera_names': [f'{i}' for i in range(14) if i % 2 == 0],
    'camera_names': [f'{i}' for i in range(14)],
    'action_names': ['Seq1', 'Seq2']
}
...
table_dtype = np.dtype([
    ...
    ('action_idx', np.int8),
    ('frame_idx', np.int16),
    ('keypoints', np.float32, (28, 3)),  # Currently there are no GT, so the shape is the same as h36m
...
for subject_idx, subject in enumerate(retval['subject_names']):
    calib_dir = os.path.join(root, subject, 'Seq1', 'calibration')
...
for subject_idx, subject in enumerate(retval['subject_names']):
    ...
    for action_idx, action in enumerate(retval['action_names']):
        bboxes[subject][action] = {}
        for camera_idx, camera in enumerate(retval['camera_names']):
            bboxes[subject][action][camera] = []
            bb_file = os.path.join(root, subject, action, 'BoundingBox', f'BoundingBox{camera}.txt')
            bb_tmp = np.loadtxt(bb_file).astype(np.float32)
            for i in range(bb_tmp.shape[0]):
                bboxes[subject][action][camera].append(square_the_bbox(bb_tmp[i]))
            bboxes[subject][action][camera] = np.vstack(bboxes[subject][action][camera])
...
for subject_idx, subject in enumerate(retval['subject_names']):
    for action_idx, action in enumerate(retval['action_names']):
        # image_path = os.path.join(root, subject, 'Images', action)
        ...
        pose_file_list = glob.glob(os.path.join(root, subject, action, 'gt3Dpose', '*.txt'))
        frame_idxs = list(range(len(pose_file_list)-1))
        # frame_idxs = list(range(0, len(pose_file_list)-1, 5))
        # frame_idxs = list(range(3000))
        print(subject, action, len(frame_idxs))
        poses_world = []
        # for pose_file in pose_file_list:
        #     pose_world = np.loadtxt(pose_file)
        #     poses_world.append([pose_world])
        for frm in frame_idxs:
            pose_world = np.loadtxt(os.path.join(root, subject, action, 'gt3Dpose', f'pose{frm:04d}.txt'))
            poses_world.append([pose_world])
        ...
        for (camera_idx, camera) in enumerate(retval['camera_names']):
            ...
            for bbox, frame_idx in zip(table_segment['bbox_by_camera_tlbr'], frame_idxs):
                try:
                    ...
                    print(frame_idx, e)
                    bbox[camera_idx] = np.array([0, 0, 0, 0], dtype=int)
...
print('Total frames in MPI-INF-3DHP:', len(retval['table']))
np.save(f'mpiinf3dhp-S1-ALLfrm-allcam-multiview-labels-bboxes.npy', retval)
