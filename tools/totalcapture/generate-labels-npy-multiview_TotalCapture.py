...
retval = {
    ...
    'subject_names': ['s1', 's4', 's5'],
    'camera_names': [f'{i}' for i in range(1, 9)],
    # 'action_names': ['acting1', 'acting2', 'acting3',
    #                  'freestyle1', 'freestyle2', 'freestyle3',
    #                  'rom1', 'rom2', 'rom3',
    #                  'walking1', 'walking2', 'walking3']    
    'action_names': ['acting3', 'freestyle3', 'walking2']
}
...
for subject_idx, subject in enumerate(retval['subject_names']):
    ...
    for action_idx, action in enumerate(retval['action_names']):
        ...
        for camera_idx, camera in enumerate(retval['camera_names']):
            ...
            bb_file = os.path.join(root, subject, 'Boundingbox_fix_manual', action, f'BoundingBox{camera}.txt')
...
for subject_idx, subject in enumerate(retval['subject_names']):
    for action_idx, action in enumerate(retval['action_names']):
        ...
        pose_file_list = glob.glob(os.path.join(root, subject, 'gt3Dpose', action, '*.txt'))
        frame_idxs = list(range(1000, len(pose_file_list)-2))
        # frame_idxs = list(range(1000))
        poses_world = []
        # for pose_file in pose_file_list:
        #     pose_world = np.loadtxt(pose_file)
        for frm in frame_idxs:
            pose_world = np.loadtxt(os.path.join(root, subject, 'gt3Dpose', action, f'pose{frm:04d}.txt'))
...
print('Total frames in TotalCapture:', len(retval['table']))
np.save(f'totalcap-s145A3FS3W2-morethan1000frm-multiview-labels-bboxes.npy', retval)
